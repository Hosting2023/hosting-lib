...
## Авторизация
```bash
npm login --registry=https://npm.pkg.github.com --scope=@hosting2023
```

В качестве пароля - токен с гитхаба. 
...
## Если есть необходимость, можно вызвать configureAPI и установить своё значение в поле baseURL
...
Возвращает: ID пользователя после успешного входа (на данный момент возвращает isAuth: true).
...
Возвращает: ID зарегистрированного пользователя (на данный момент возвращает isAuth: true).
